...
export function BankSelector({ 
  ...
  } = useBusinessBanks(businessId)

  // Filter banks based on input
  const filterBanks = useCallback((searchValue: string) => {
    if (!searchValue.trim()) {
      return banks
    }
    return banks.filter((bank: BankItem) =>
      bank.name.toLowerCase().includes(searchValue.toLowerCase())
    )
  }, [banks])

  // Update filtered banks when input changes
  useEffect(() => {
    const filtered = filterBanks(inputValue)
    setFilteredBanks(filtered)
    setActiveSuggestionIndex(-1)
  }, [inputValue, filterBanks])
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Notify parent that no bank is selected when typing
    onBankChange(null)
  }
  ...
  const handleInputFocus = () => {
    if (!disabled) {
      setIsOpen(true)
      setFilteredBanks(filterBanks(inputValue))
    }
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (!isOpen || filteredBanks.length === 0) return
    ...
    switch (e.key) {
      ...
      case "Enter":
        e.preventDefault()
        if (activeSuggestionIndex >= 0 && filteredBanks[activeSuggestionIndex]) {
          selectBank(filteredBanks[activeSuggestionIndex])
        }
        break
      case "Escape":
        setIsOpen(false)
    ...
    <div className={`space-y-2 ${className}`} ref={containerRef}>
      ...
      <div className="relative">
        <Input
          ...
          disabled={disabled}
          className={`pr-10 ${selectedBankId ? 'border-green-500' : ''} ${error ? 'border-red-500 focus:border-red-500' : ''}`}
          autoComplete="off"
        />
          ...
          >
            {filteredBanks.length > 0 ? (
              ...
              <div className="px-4 py-3 text-gray-500 text-center">
                {inputValue ? 'No matching banks found' : 'No banks available'}
              </div>
            )}
          </div>
        )}
      ...
      {/* Empty banks warning */}
      {banks.length === 0 && !isLoading && !bankError && (
        <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
