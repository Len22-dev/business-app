...
import { categoryQueries } from '@/lib/drizzle/queries/category-queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await categoryQueries.getMany(businessId, filters, { page, limit });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const categoryData = data.categoryData || data;
    
    const category = await categoryQueries.create(categoryData);
